...
void wxSDKEditorFrame::CreateWorkspace()
{
    ...
    const IEditorTree::ItemHandle root = editorTree_->AddRoot("Scene");
    editorTree_->AddChild(root, "Objects placeholder");
    editorTree_->ExpandAll();
}
...
void wxSDKEditorFrame::OnAbout(wxCommandEvent&)
{
    dialogService_.Info("About wxSDKEditor",
        "Experimental wxWidgets shell for gradual X-Ray SDK UI restoration.");
}
