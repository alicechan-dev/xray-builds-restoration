...
| MagicFM SDK | Incomplete | Runtime has `MagicFM.dll` and `MagicFMDLLB.dll`; repository has historical `MagicFM.lib` files, but no matching SDK headers were found. | Existing opt-in `XR_MAGICFM_ROOT` diagnostics; do not claim usability until headers are found. |
