...
| MagicFM SDK | Incomplete | Runtime has `MagicFM.dll` and `MagicFMDLLB.dll`; repository has historical `MagicFM.lib` files, including `Editor/ShaderEditor/Lib/MagicFM.lib`, but no matching SDK headers were found. ParticleEditor asks for `MagicFMDLLB.lib`, which is not present in this snapshot. | Existing opt-in `XR_MAGICFM_ROOT` diagnostics; do not claim usability until headers are found. |
...
LevelEditor, LevelOptions, ShaderEditor, and ParticleEditor shells.
