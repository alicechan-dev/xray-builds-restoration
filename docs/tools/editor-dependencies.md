...
| FreeImage | Partial / blocked | `FreeImage.h` copies exist in the repo and `D:\Projects\Github\stalker-dream\FreeImage.dll` is a 32-bit PE DLL. The only in-repo import library, `Editor/ShaderEditor/Lib/FreeImage.lib`, is not a valid MSVC COFF archive and is likely a legacy non-COFF import library. | New opt-in `XR_FREEIMAGE_ROOT` or direct `XR_DO_LIGHT_FREEIMAGE_LIB` for `xrDO_Light`; see [FreeImage Dependency](freeimage-dependency.md) for the local import-library generation workflow. Do not vendor FreeImage files. |
...
| AlexMX Borland controls | Not found | No `ExtBtn.hpp`, `MXCtrls.hpp`, `MxMenus.hpp`, or `mxPlacemnt.hpp`. The repository's `AlexMX/` tree is GemVIPM geometry code, not the control package. | Existing opt-in `XR_ALEXMX_ROOT` diagnostics; keep GUI targets blocked when absent. |
| MagicFM SDK | Incomplete | Runtime has `MagicFM.dll` and `MagicFMDLLB.dll`; repository has historical `MagicFM.lib` files, including `Editor/ShaderEditor/Lib/MagicFM.lib`, but no matching SDK headers were found. ParticleEditor and ActorEditor ask for `MagicFMDLLB.lib`, which is not present in this snapshot. | Existing opt-in `XR_MAGICFM_ROOT` diagnostics; do not claim usability until headers are found. |
...
| Borland C++ Builder/VCL | Not found | No Borland/Embarcadero component root under the targeted installed-program paths; no `R:` drive exists. | Existing opt-in `XR_BORLAND_ROOT` diagnostics; do not fake VCL headers or package behavior. |
...
- whether `XR_ALEXMX_ROOT` is unset or has candidate control headers;
- whether `XR_MAGICFM_ROOT` is unset or has candidate SDK headers;
- whether `XR_BORLAND_ROOT` is unset or has candidate `include` and
  `include/vcl` directories;
...
The AlexMX, MagicFM, and Borland cache variables are diagnostics and future
target-local probes only. They do not vendor packages, add paths globally, or
prove MSVC compatibility.
...
LevelEditor, LevelOptions, ShaderEditor, ParticleEditor, and ActorEditor
shells.
