...
| `Gauss` | `BUILD_XR_GAUSS` | Configures and gets past missing-header, dependent-type, and Gauss-local `std::binary_function` blockers; build now stops at stale absent `Log.h`/`Engine.h` PCH includes. |
