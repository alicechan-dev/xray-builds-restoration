...
| Editor core | `Editors/ECore/` | Shared editor library | Shared editor engine, thumbnails, object IO, properties, image helpers, and shader helpers. | Opt-in CMake target `xrECore` added as an experimental partial port. VCL form units are excluded for now. | `xrCore`, `xrCDB`, `xrParticles`, `ETools`, DirectX 9/D3DX, `xrEProps`, Borland/VCL packages, ElPack/AlexMX, MagicFM. | Configure passes with `BUILD_XR_ECORE=ON`; low-level compatibility blockers are fixed. The retained source/API surface still depends pervasively on `AnsiString`, `TShiftState`, `TMsgDlgType`, and ElPack, so no additional files can be honestly excluded without defining a non-VCL library boundary first. | Foundation for restored editors. | High | See [ECore CMake Port Inventory](ecore-inventory.md). |
