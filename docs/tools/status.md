...
| LWO tool | `Editors/Tools/LWO/` | Converter/helper DLL | LightWave object/envelope handling utility. | Experimental opt-in `LWO` CMake shell added behind `BUILD_XR_LWO`; legacy project source ownership is represented without inventing missing files. | LightWave object parsing code. | Configure passes; build stops honestly at missing `lwo2.h` / missing historical LightWave reader sources. | Import/export pipeline support. | Medium | See [LWO CMake Shell](lwo-cmake.md). |
