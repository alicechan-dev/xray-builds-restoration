...
| Editor core | `Editors/ECore/` | Shared editor library | Shared editor engine, thumbnails, object IO, properties, image helpers, and shader helpers. | Opt-in CMake target `xrECore` added as an experimental partial port. VCL form units are excluded for now. | `xrCore`, `xrCDB`, `xrParticles`, `ETools`, DirectX 9/D3DX, `xrEProps`, Borland/VCL packages, ElPack/AlexMX, MagicFM. | Configure passes with `BUILD_XR_ECORE=ON`; `fastmath.h`, `utime.h`, CRT wrapper redefinitions, `_FARQ`, and DirectSound wave-format declaration blockers are fixed; current blocker is the Borland/VCL/editor package boundary. | Foundation for restored editors. | High | See [ECore CMake Port Inventory](ecore-inventory.md). |
