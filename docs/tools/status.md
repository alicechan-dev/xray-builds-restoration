...
| wxWidgets SDK editor shell | `Editors/wxSDKEditor/` | Experimental GUI shell | Tests a parallel, portable SDK UI boundary without porting historical editor behavior. | Opt-in `wxSDKEditor` target behind `BUILD_XR_WX_SDK_EDITOR`; wx-free `EditorTreePresenter` coordinates `EditorTreeModel`, `IEditorTree`, properties, and dialogs. Rename/create/delete, strict atomic `.wx_tree_snapshot` v1 save/load, and a development-only `.wx_tree_paths` importer remain model-backed. | External wxWidgets built for Win32/x86, normally supplied through vcpkg `wxwidgets:x86-windows`. | Normal configuration is unaffected while OFF; the opt-in Win32 shell builds and passes a no-data launch smoke check. Separate `BUILD_XR_WX_SDK_EDITOR_MODEL_TESTS` / `wxSDKEditorModelTests` builds and passes without wxWidgets, using fake tree/property/dialog adapters to cover presenter coordination. | Foundation for incremental, testable UI migration after historical data/command contracts are understood. | Experimental | `MainFrame` is wx layout/event/file-dialog glue; headless tests cover presenter coordination, delete confirmation, atomic failed import, model mutation, path-list import, and malformed snapshots without assets or runtime data. Neither development format is a real SDK format, and no `xrEProps` or historical editor logic is loaded. See [Experimental wxWidgets SDK Editor](wx-sdk-editor.md) and [ElPack Migration Map](wx-editor-elpack-migration.md). |
