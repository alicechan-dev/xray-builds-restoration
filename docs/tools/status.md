...
| Editor core | `Editors/ECore/` | Shared editor library | Shared editor engine, thumbnails, object IO, properties, image helpers, and shader helpers. | Opt-in CMake target `xrECore` added as an experimental partial port. VCL form units are excluded for now. An isolated `AnsiString` contract target exists but is not integrated. | `xrCore`, `xrCDB`, `xrParticles`, `ETools`, DirectX 9/D3DX, `xrEProps`, Borland/VCL packages, ElPack/AlexMX, MagicFM. | Configure passes with `BUILD_XR_ECORE=ON`; low-level compatibility blockers are fixed. The retained source/API surface still depends pervasively on `AnsiString`, `TShiftState`, `TMsgDlgType`, and ElPack. `BUILD_XR_VCL_COMPAT_TESTS` builds the standalone `vcl_ansi_string_contract`; `xrECore` remains blocked intentionally. | Foundation for restored editors. | High | See [ECore CMake Port Inventory](ecore-inventory.md) and [VCL Compatibility Strategy](vcl-compat-strategy.md). |
