...
| Gauss tool | `Editor/Tools/gauss/` | Image-filter helper DLL | Exposes a Gaussian kernel plus kernel normalization and RGBA filtering helpers. | Experimental opt-in `Gauss` CMake target added behind `BUILD_XR_GAUSS`; uses the complete VC6 project rather than the incomplete later snapshot. | Target-local DirectX 8-era headers/libraries. | Configure passes. Stale absent PCH includes are removed after confirming they are unused; build now stops at removed VC6 STL implementation detail `std::_cpp_max`. | Small non-GUI image-processing helper. | Low | See [Gauss CMake Shell](gauss-cmake.md). |
...
| `Gauss` | `BUILD_XR_GAUSS` | Configures and gets past stale PCH includes after confirming they are unused. | Removed VC6 STL implementation detail `std::_cpp_max` in `_vector3d.h`. |
