...
| wxWidgets SDK editor shell | `Editors/wxSDKEditor/` | Experimental GUI shell | Tests a parallel, portable SDK UI boundary without porting historical editor behavior. | Foundation checkpoint established behind opt-in `BUILD_XR_WX_SDK_EDITOR`: wx-free `EditorTreePresenter` coordinates `EditorTreeModel`, `IEditorTree`, properties, and dialogs. Create/rename/delete, strict atomic `.wx_tree_snapshot` v1 save/load, and development-only `.wx_tree_paths` import are model-backed. | External wxWidgets built for Win32/x86, normally supplied through vcpkg `wxwidgets:x86-windows`. | Normal configuration is unaffected while OFF; the opt-in Win32 shell builds. Separate `BUILD_XR_WX_SDK_EDITOR_MODEL_TESTS` / `wxSDKEditorModelTests` builds and passes without discovering, including, or linking wxWidgets, using fake tree/property/dialog adapters. | Tested seam for future category mapping and read-only metadata adapters; not a replacement for the Borland SDK. | Experimental | `MainFrame` is wx layout/event/file-dialog glue; headless tests cover presenter coordination, delete confirmation, atomic failed import, model mutation, path-list import, and malformed snapshots. No real SDK data is loaded, and no `xrEProps`, LevelEditor, or runtime-heavy source is touched. See [Experimental wxWidgets SDK Editor](wx-sdk-editor.md) and [ElPack Migration Map](wx-editor-elpack-migration.md). |
