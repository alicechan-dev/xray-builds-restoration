...
| Gauss tool | `Editor/Tools/gauss/` | Image-filter helper DLL | Exposes a Gaussian kernel plus kernel normalization and RGBA filtering helpers. | Experimental opt-in `Gauss` CMake target added behind `BUILD_XR_GAUSS`; uses the complete VC6 project rather than the incomplete later snapshot. | Target-local DirectX 8-era headers/libraries. | Configure passes. Component-wise min/max now use standard helpers; build stops at unresolved `Msg(...)` calls whose logging declaration/implementation is absent from the canonical snapshot. | Small non-GUI image-processing helper. | Low | See [Gauss CMake Shell](gauss-cmake.md). |
...
| `Gauss` | `BUILD_XR_GAUSS` | Configures and gets past removed `_cpp_min`/`_cpp_max` implementation details with standard helpers. | Undeclared `Msg(...)` calls in `_math.cpp` and `FS.cpp`; logging source ownership needs review. |
