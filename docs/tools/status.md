...
| Gauss tool | `Editor/Tools/gauss/` | Image-filter helper DLL | Exposes a Gaussian kernel plus kernel normalization and RGBA filtering helpers. | Experimental opt-in `Gauss` CMake target added behind `BUILD_XR_GAUSS`; uses the complete VC6 project rather than the incomplete later snapshot. | Target-local DirectX 8-era headers/libraries. | Configure passes. Component-wise maximum now uses `std::max`; build advances to the adjacent removed `std::_cpp_min` implementation detail. | Small non-GUI image-processing helper. | Low | See [Gauss CMake Shell](gauss-cmake.md). |
...
| `Gauss` | `BUILD_XR_GAUSS` | Configures and gets past the removed `std::_cpp_max` implementation detail with `std::max`. | Adjacent removed `std::_cpp_min` in `_vector3d.h`. |
