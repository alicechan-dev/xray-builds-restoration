...
| Editor core | `Editors/ECore/` | Shared editor library | Shared editor engine, thumbnails, object IO, properties, image helpers, and shader helpers. | Opt-in CMake target `xrECore` added as an experimental partial port. VCL form units are excluded for now. The tested `AnsiString` shim is integrated for non-Borland builds only. | `xrCore`, `xrCDB`, `xrParticles`, `ETools`, DirectX 9/D3DX, `xrEProps`, Borland/VCL packages, ElPack/AlexMX, MagicFM. | Configure passes with `BUILD_XR_ECORE=ON`; low-level compatibility and `AnsiString` blockers are addressed. `BUILD_XR_VCL_COMPAT_TESTS` builds the standalone contract. Remaining VCL UI types and ElPack keep `xrECore` intentionally incomplete. | Foundation for restored editors. | High | See [ECore CMake Port Inventory](ecore-inventory.md) and [VCL Compatibility Strategy](vcl-compat-strategy.md). |
