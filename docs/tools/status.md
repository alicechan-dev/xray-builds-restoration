...
| wxWidgets SDK editor shell | `Editors/wxSDKEditor/` | Experimental GUI shell | Tests a parallel, portable SDK UI boundary without porting historical editor behavior. | Foundation checkpoint established behind opt-in `BUILD_XR_WX_SDK_EDITOR`: wx-free `EditorTreePresenter` coordinates `EditorTreeModel`, neutral `EditorItemKind`, tree, properties, and dialogs. Create/rename/delete, v2 snapshots with v1 reader compatibility, and development-only path-list import are model-backed. | External wxWidgets built for Win32/x86, normally supplied through vcpkg `wxwidgets:x86-windows`. | Normal configuration is unaffected while OFF; the opt-in Win32 shell builds. Separate `BUILD_XR_WX_SDK_EDITOR_MODEL_TESTS` / `wxSDKEditorModelTests` passes without discovering, including, or linking wxWidgets. | Tested model seam for later consumer-specific audits and read-only metadata adapters; not a replacement for the Borland SDK. | Experimental | `EditorItemKind` maps only audited root/folder/object structure and preserves unknown categories without guessing. Tests cover kind conversion, v1/v2 persistence, import inference, presenter coordination, and malformed input. No real SDK data is loaded, and no `xrEProps`, LevelEditor, or runtime-heavy source is touched. See [Item Types](wx-editor-item-types.md), [wxSDKEditor](wx-sdk-editor.md), and [ElPack Migration Map](wx-editor-elpack-migration.md). |
