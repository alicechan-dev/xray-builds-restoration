...
#	if !(defined(__BORLANDC__) || defined(NO_XRNEW))
	IC void*	__cdecl operator new		(size_t size)		{	return Memory.mem_alloc(size?size:1, "C++ NEW");	}
...
#else
#	if !(defined(__BORLANDC__) || defined(NO_XRNEW))
	IC void*	__cdecl operator new		(size_t size)		{	return Memory.mem_alloc(size?size:1);				}
	IC void		__cdecl operator delete		(void *p)			{	xr_free(p);											}
	IC void*	__cdecl operator new[]		(size_t size)		{	return Memory.mem_alloc(size?size:1);				}
	IC void		__cdecl operator delete[]	(void* p)			{	xr_free(p);											}
#	endif
